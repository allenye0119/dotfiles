...
spaceship_host() {
  ...
  [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "%{$fg[white]%}${SPACESHIP_PREFIX_HOST}" || echo -n ' '
  # Display machine name
  echo -n "%{$fg[green]%}%m%{$reset_color%}"
  # Do not show host prefix if prefixes are disabled
  [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "%{$fg[white]%}${SPACESHIP_PREFIX_DIR}" || echo -n ' '
}
...
spaceship_git_status() {
  ...
  if [[ "$(git rev-parse --is-inside-git-dir 2> /dev/null)" == 'false' ]]; then
    ...
    [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "%{$fg[white]%}${SPACESHIP_PREFIX_GIT}" || echo -n ' '
